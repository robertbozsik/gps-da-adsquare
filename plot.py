...
def plot_a_store(name):
    ...
    # save the plot
    sns_plot.figure.savefig(f"./out_data/unique_visits_of_{name}.png")
    # display the plot
    plt.show()
...
if __name__ == "__main__":
    if len(sys.argv) == 2:
        ...
        print("""
        python(3) plot.py $params
        e.g. python(3) plot.py plot_all_stores (Plot and save the unique visits of each sotre.)
             python(3) plot.py plot_a_store Aldi (Plot and save the unique visits of the selected store.)
